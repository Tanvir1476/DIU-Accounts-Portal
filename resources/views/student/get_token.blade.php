    ...
    <h2 class="font-normal text-l text-gray-800 ">
        ...
        <div class="container">
            ...
            <form method="POST" action="{{ route('fee.request') }}">
                ...
                @else
                    <button type="button" class="button" style="background:gray;cursor:not-allowed;">
                         Closed (Open 9AM-4PM)
                    </button>
                @endif
                ...
                @if (session('error'))
                    <div class="token-box">
                        <b style="color: red;">{{ session('error') }}</b><br><a href="/contact"
                            style="text-decoration: underline;">Contact Accounts Office</a>
                    </div>
                @endif
